class AlbumImagesController < ApplicationController
  ...
  def create
    ...
    if @album_image.save
      redirect_to @album_image, :notice => "Successfully created album image."
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @album_image.update_attributes(params[:album_image])
      redirect_to @album_image, :notice  => "Successfully updated album image."
    else
      render :action => 'edit'
    end
  end
  ...
  def destroy
    ...
    @album_image.destroy
    redirect_to :controller => "albums",:action => "show_images",:id =>@album_id , :notice => "Successfully destroyed album image."
